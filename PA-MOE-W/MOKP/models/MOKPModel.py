...
class KPModel(nn.Module):
    ...
    def forward(self, state):
        ...
        if state.current_node is None:
            ...
            probs, moe_loss = self.decoder(self.encoded_graph, state.capacity, self.embedded_pref, ninf_mask=state.ninf_mask)
...
class KP_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.embedding = MoE(input_size=3, input_size_pref=embedding_dim, output_size=embedding_dim, num_experts=self.model_params['num_experts'],
...
class EncoderLayer(nn.Module):
    def __init__(self, **model_params):
        ...
        self.Wq_pref = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
...
class KP_Decoder(nn.Module):
    ...
    def forward(self, graph, capacity, embedded_pref,  ninf_mask):
        ...
        mh_atten_out, loss = self.multi_head_combine(out_concat, embedded_pref)
        # shape: (batch, pomo, embedding)
