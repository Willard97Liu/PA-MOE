...
class CVRPModel(nn.Module):
    ...
    def pre_forward(self, reset_state, mid_embd_pref):
        ...
        self.encoded_nodes, moe_loss = self.encoder(depot_xy, node_xy_demand, mid_embd_pref)
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs, moe_loss = self.decoder(encoded_last_node, self.mid_embd_pref, ninf_mask=state.ninf_mask)
...
class CVRP_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        # self.embedding_node = MoE(input_size=3, input_size_pref=8, output_size=embedding_dim, num_experts=self.model_params['num_experts'],
    ...
    def forward(self, depot_xy, node_xy_demand, mid_embd_pref):
        
        moe_loss = 0
        # if isinstance(self.embedding_depot, MoE) or isinstance(self.embedding_node, MoE):
            
        #     embedded_depot = self.embedding_depot(depot_xy)
        #     embedded_node, loss_node = self.embedding_node(node_xy_demand, mid_embd_pref)
...
class EncoderLayer(nn.Module):
    def __init__(self, **model_params):
        ...
        # 修改这个为MOE结构
        self.feed_forward = MoE(input_size=embedding_dim, input_size_pref=8, output_size=embedding_dim, num_experts=self.model_params['num_experts'],
    ...
    def forward(self, input1, mid_embd_pref):
        ...
        out2, moe_loss = self.feed_forward(out1, mid_embd_pref)
